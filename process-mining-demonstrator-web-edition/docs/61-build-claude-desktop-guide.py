#!/usr/bin/env python3
"""Generate the self-contained HTML guide
"61 - Conversational Process Mining with Claude Desktop".

The user's side of Conversational Process Mining: add the Process Mining MCP server as a
custom Connector in Claude Desktop, sign in via OAuth, and start asking — with a set of
example queries and follow-ups. Assumes an admin has already enabled the MCP server
(guide 15). Facts from MCP-SERVER.md and mcp/server.py.

Run:  python3 docs/61-build-claude-desktop-guide.py
"""
import base64
import html
import pathlib
...
OUT = ROOT / "docs" / "61-Conversational-Process-Mining-Claude-Desktop.html"
...
STEPS = [
    (
        "Before you start",
        "This guide is the <b>user side</b> of Conversational Process Mining — connecting "
        "Claude Desktop to the Process Mining <b>MCP server</b>. A few things must already be "
        "true (an administrator handles the first three; see guide 15):",
        None,
        [
            ...
            "You have <b>Claude Desktop</b> installed and are signed in to Claude.",
        ],
        '<span class="k">Know your server URL.</span> Ask your administrator for the exact MCP '
        "URL (the direct <code>:18493/mcp</code> or the public proxy path). You'll paste it in "
        "the next step.",
    ),
    (
        "Add the Process Mining connector",
        "Claude Desktop calls MCP servers <b>Connectors</b>. Add the Process Mining server as a "
        "custom connector once; it stays available afterwards.",
        """<div class="mock">
      <table class="tbl">
        <tr><th>#</th><th>In Claude Desktop</th></tr>
        <tr><td>1</td><td>Open <b>Settings → Connectors</b>.</td></tr>
        <tr><td>2</td><td>Click <b>Add custom connector</b>.</td></tr>
        <tr><td>3</td><td><b>Name</b>: <code>Process Mining</code>.</td></tr>
        <tr><td>4</td><td><b>URL</b>: the MCP endpoint your admin gave you, e.g. <code>https://&lt;host&gt;:18493/mcp</code>.</td></tr>
        <tr><td>5</td><td>Click <b>Add</b> / <b>Save</b>, then <b>Connect</b>.</td></tr>
      </table>
      <p class="cap">Claude reads the server's <code>/.well-known/oauth-protected-resource</code>
        document, discovers your OAuth provider, and opens its sign-in page.</p>
    </div>""",
        None,
        None,
    ),
    (
      ...
      <p class="cap">After you approve, the connector shows as connected and its tools
        ...
        None,
        '<span class="k">Signed in as you.</span> Whoever authenticates must match an enabled '
        "Process Mining user. Claude then sees exactly that user's assigned connections — "
        "nothing more. If sign-in fails or says no user matches, see Troubleshooting.",
    ),
    (
        "Enable it in a chat",
        "Connectors are enabled per conversation. Start a new chat, switch the <b>Process "
        "Mining</b> connector on (the connector/tools control in the message box), and you're "
        "ready to ask. The first time the agent calls a tool, Claude may ask you to allow it — "
    ...
    (
        ...
        "You never name the tools; ask in plain language and the agent picks them. Good openers, "
        "roughly in the order you'd explore a new process:",
        """<div class="mock">
      <p class="cap"><b>Orient yourself</b></p>
      <pre class="calc">"List my process-mining connections and the projects on each."
"Which project has the most journeys?"
"What do the meta attributes mean in project 1, and what date range does the data cover?"</pre>
      <p class="cap"><b>Understand the process</b></p>
      <pre class="calc">"Show me the process map for project 1 on connection Prod."
"What are the ten most common paths, and how often does each happen?"
"Which transitions are the slowest by median time?"</pre>
      <p class="cap"><b>KPIs &amp; health</b></p>
      <pre class="calc">"How many journeys in the last 30 days, and what's the process-goodness score?"
"Give me the average and median end-to-end duration."</pre>
...
"Show me the shared URGENT and IMPORTANT notes, but not my personal ones."</pre>
        ...
        "(&ldquo;on connection <i>Prod</i>, project 1&rdquo;) so the agent doesn't have to "
        "guess.",
    ),
    (
        "Keep the conversation going",
        "The value is in the follow-ups — each answer sets up the next question, and the agent "
        "chains the tools for you across the whole chat.",
        None,
        [
            "<b>Narrow it</b> — &ldquo;only March&rdquo;, &ldquo;just the cases where department "
            "= Finance&rdquo;, &ldquo;exclude journeys that hit <i>Cancelled</i>&rdquo;.",
            "<b>Compare</b> — &ldquo;how does the sample set compare with the full data?&rdquo;",
            "<b>Explain</b> — &ldquo;why is that transition slow — show me a few example "
            "cases&rdquo;.",
            "<b>Summarise</b> — &ldquo;write a short bottleneck summary I can paste into a "
            "ticket&rdquo;.",
        ],
        None,
    ),
    (
        "Troubleshooting",
        "If a connection or a query doesn't behave, it's almost always one of these:",
        """<div class="mock">
      <table class="tbl">
        ...
        None,
        '<span class="k">Two-minute smoke test.</span> An admin can confirm the server from a '
        "shell: <code>curl -k https://&lt;host&gt;:18493/health</code> should return "
        "<code>{&quot;ok&quot;:true,&quot;enabled&quot;:true}</code>.",
    ),
]
...
<title>61 - Conversational Process Mining with Claude Desktop</title>
...
<header class="hero"><div class="hero-inner">
  ...
  <h1>Claude Desktop: connect &amp; ask</h1>
  <p>Add the Process Mining <b>MCP server</b> as a Connector in Claude Desktop, sign in with
     OAuth, and start asking about your processes in plain language — with example queries to
     get you going.</p>
</div></header>
...
<div class="wrap">
  ...
  <div class="lead">
    This is everything you need to <b>start working with Conversational Process Mining</b> from
    Claude Desktop: add the connector, sign in through your OAuth provider, and ask. It assumes
    an administrator has already enabled the MCP server (<b>guide 15</b>); for the concept and
    what you can ask, see <b>guide 60</b>.
  </div>
...
{beyond_items}
    ...
    <p class="note">Once connected, everything is a conversation — ask, read the answer, drill
      in, and follow up.</p>
  </div>

  <footer>Process Mining Demonstrator · Suite — Conversational Process Mining with Claude Desktop</footer>
</div>
</body></html>
"""
